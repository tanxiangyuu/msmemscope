...
- Quick analysis APIs
    ...
    **Table 1** API list <a id="API list-1"></a>
    ...
    |enable_npu_sanitizer|NPU Sanitizer enablement API provided by msMemScope, activating mstx instrumentation for custom operators with the native Sanitizer.|   
...
`Dict[str, Any]`: Dictionary of all parameters, which can be directly printed.
...
This is the NPU Sanitizer enablement API provided by msMemScope, Internally, this API calls `torch_npu.npu._sanitizer.enable_npu_sanitizer()` to enable the native Sanitizer, while simultaneously notifying the C++ layer to activate the capture of mstx instrumentation messages with the `sanitizer-op:` prefix. After enablement, information reported by developers via the mstx API at custom operator call sites in the required format will be intercepted and converted into kernel launch events, which are then fed into the native Sanitizer analysis pipeline for synchronous error detection.

For more details, see [NPU Sanitizer](../user_guide/memory_analysis.md#npu-sanitizer).
...
# After enablement, report operator information at custom operator call sites via mstx.mark() according to the specification
# Pass torch_npu.npu.current_stream().npu_stream as the second parameter  to specify the stream for instrumentation
import mstx
import torch_npu
mstx.mark(
