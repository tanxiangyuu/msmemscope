...
2. 执行完成后，会出现如下两种回显信息。
    - 如果出现如[**图 1**  内存泄漏](#内存泄漏)的回显信息，表示存在内存泄漏问题。回显信息中分别展示了每张卡内存泄漏的汇总信息，包括泄漏发生的Step数、关联的kernel、地址以及泄漏大小等信息。

        **图 1**  内存泄漏 <a id="内存泄漏"></a>
        ![](figures/内存泄漏.png "内存泄漏")
...
3. 执行以下命令，调用Python接口，对输出文件中的csv文件进行离线泄漏分析。
    ...
    如果出现[**图 3**  离线泄漏分析](#离线泄漏分析)的回显信息，表示存在内存泄漏问题。

    **图 3**  离线泄漏分析 <a id="离线泄漏分析"></a>  
    ![](figures/离线泄漏分析.png "离线泄漏分析")
