...
void *LibLoadFromDriver(std::string libName)
{
    ...
    const char *ldLibPath = std::getenv("LD_LIBRARY_PATH");
    if (!ldLibPath || std::string(ldLibPath).empty()) {
        std::cout << "[msleaks] Failed to acquire LD_LIBRARY_PATH environment variable while loading "
            << libName << "."
            << std::endl;
        return nullptr;
    }

    std::string ldLibPathStr = ldLibPath;
    std::string driverPath("driver/lib64/driver");

    // 按照路径分隔符（：）分割LD_LIBRARY_PATH
    size_t pos = 0;
    std::string delimiter = ":";
    while ((pos = ldLibPathStr.find(delimiter)) != std::string::npos) {
        std::string path = ldLibPathStr.substr(0, pos);
        if (path.find(driverPath) != std::string::npos) {
            path += "/" + libName;
            return dlopen(path.c_str(), RTLD_NOW | RTLD_GLOBAL);
        }
        ldLibPathStr.erase(0, pos + delimiter.length());
    }

    // 检查最后一部分路径
    if (ldLibPathStr.find(driverPath) != std::string::npos) {
        ldLibPathStr += libName;
        return dlopen(ldLibPathStr.c_str(), RTLD_NOW | RTLD_GLOBAL);
    }
    return nullptr;
}
