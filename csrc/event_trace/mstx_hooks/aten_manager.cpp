...
void AtenManager::ReportAtenLaunch(const char* msg, int32_t streamId, bool isAtenBegin)
{
    ...
    if (!EventReport::Instance(LeaksCommType::DOMAIN_SOCKET).ReportAtenLaunch(buffer)) {
...
void AtenManager::ReportAtenAccess(const char* msg, int32_t streamId)
{
    ...
    if (!EventReport::Instance(LeaksCommType::DOMAIN_SOCKET).ReportAtenAccess(buffer)) {
