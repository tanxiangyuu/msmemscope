...
namespace Leaks {
// 组装普通打点信息
void MstxManager::ReportMarkA(const char* msg)
{
    MstxRecord record;
    record.markType = MarkType::MARK_A;
    record.rangeId = onlyMarkId_;

    if (EOK != strncpy_s(record.markMessage, sizeof(record.markMessage), msg, sizeof(record.markMessage) - 1)) {
...
// 组装Range开始打点信息
uint64_t MstxManager::ReportRangeStart(const char* msg)
{
    MstxRecord record;
...
void MstxManager::ReportRangeEnd(uint64_t id)
{
    ...
    if (EOK != strncpy_s(record.markMessage, sizeof(record.markMessage), msg.c_str(), sizeof(record.markMessage) - 1)) {
        Utility::LogError("strncpy_s FAILED");
    }
    record.markMessage[sizeof(record.markMessage) - 1] = '\0';
    if (!EventReport::Instance(CommType::SOCKET).ReportMark(record)) {
