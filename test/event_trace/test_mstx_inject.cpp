...
TEST(InitInjectionMstxTest, OutTableIsNull) {
    auto mockGetFuncTable = [](MstxFuncModule, MstxFuncTable* outTable, unsigned int* outSize) -> int {
        ...
        *outSize = static_cast<unsigned int>(MstxFuncSeq::MSTX_FUNC_END);
        return MSTX_SUCCESS;
    };
    EXPECT_EQ(InitInjectionMstx(mockGetFuncTable), MSTX_FAIL);
}

// 4. 测试 outSize 不等于 MSTX_FUNC_END 的情况
TEST(InitInjectionMstxTest, OutSizeNotEqualFuncEnd) {
    auto mockGetFuncTable = [](MstxFuncModule, MstxFuncTable* outTable, unsigned int* outSize) -> int {
        static MstxFuncPointer funcArray[3];  // 模拟一个大小为 3 的函数表
        static MstxFuncPointer* funcTable[] = {funcArray, funcArray + 1, funcArray + 2};
        *outTable = funcTable;
        *outSize = 2;  // 设置 outSize 为不正确的值
        return MSTX_SUCCESS;
    };
    EXPECT_EQ(InitInjectionMstx(mockGetFuncTable), MSTX_FAIL);
}
