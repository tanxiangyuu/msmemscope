...
本案例介绍了内存分析工具msMemScope的内存块监测功能，目前主要用于内存踩踏的定位。大模型场景下，每张卡的计算任务十分复杂，如果内存踩踏一旦发生，定位非常困难。msMemScope工具支持通过命令行和python接口在算子执行前后监测指定内存块，根据内存块数据的变化，定位算子间内存踩踏的范围或具体位置。
...
2. 依赖Aten算子访问事件，因此需要TorchNPU>=2.3.1。
